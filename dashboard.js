  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function showHoldingsSkeleton() {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        row.className = 'holding-row holding-skeleton animate-pulse p-md border border-outline-variant/10 rounded-2xl bg-white dark:bg-dark-surface-container-high/40 mb-sm flex flex-col justify-between';
        row.innerHTML = `
    ...
    function showNewsSkeleton(listElement) {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        card.className = 'news-skeleton animate-pulse p-sm flex gap-md items-center border border-outline-variant/10 rounded-xl bg-white dark:bg-dark-surface-container';
        card.innerHTML = `
    ...
    function renderHoldings(holdings, summary) {
      ...
      holdings.forEach((h) => {
        ...
        let returnColorClass = 'text-on-surface-variant dark:text-dark-on-surface-variant';
        if (h.gain > 0) {
        ...
        const row = document.createElement('div');
        row.className = 'holding-row p-md border border-outline-variant/20 dark:border-outline-variant/10 rounded-2xl bg-white dark:bg-dark-surface-container-high/60 shadow-xs hover:shadow-md transition-all duration-200 flex flex-col justify-between mb-sm';
        ...
        row.innerHTML = `
          ...
          <div class="grid grid-cols-12 gap-xs sm:gap-sm items-start w-full min-w-0">
            ...
            <div class="col-span-5 min-w-0 flex flex-col justify-center pr-1">
              <span class="text-[11px] sm:text-xs font-semibold uppercase tracking-wider text-on-surface-variant/70 dark:text-dark-on-surface-variant/70 mb-1 block">Stock Name</span>
              <div class="font-bold text-label-md sm:text-body-md text-on-surface dark:text-dark-on-surface leading-snug break-words" title="${companyDisplayName}">
                ${companyDisplayName}
              </div>
              ${showTickerSubtle ? `<span class="text-[11px] font-normal text-on-surface-variant/60 dark:text-dark-on-surface-variant/60 block mt-0.5">${h.symbol}</span>` : ''}
            </div>

            <!-- Column 2: Current Price (3 cols out of 12) -->
            <div class="col-span-3 min-w-0 flex flex-col justify-center">
              <span class="text-[11px] sm:text-xs font-semibold uppercase tracking-wider text-on-surface-variant/70 dark:text-dark-on-surface-variant/70 mb-1 block">Current Price</span>
              <div class="font-bold text-label-md sm:text-body-md text-on-surface dark:text-dark-on-surface whitespace-nowrap">
                ${currentPriceStr}
              </div>
            </div>
            ...
            <div class="col-span-4 min-w-0 flex flex-col justify-center">
              <span class="text-[11px] sm:text-xs font-semibold uppercase tracking-wider text-on-surface-variant/70 dark:text-dark-on-surface-variant/70 mb-1 block">Return</span>
          ...
          <div class="flex items-center justify-end gap-sm mt-md pt-sm border-t border-outline-variant/10">
            <button class="edit-holding-btn px-3 py-1.5 bg-surface-container-low hover:bg-surface-container-high dark:bg-dark-surface-container dark:hover:bg-surface-container-highest text-on-surface dark:text-dark-on-surface border border-outline-variant/30 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 shadow-2xs" data-id="${h.id}" aria-label="Edit holding ${h.symbol}">
    ...
    async function loadWatchlist() {
      try {
        ...
        data.watchlist.forEach((item) => {
          ...
          const row = document.createElement('div');
          row.className = 'watchlist-row flex justify-between items-center p-sm border border-outline-variant/20 rounded-xl hover:bg-surface-container/30 dark:hover:bg-dark-surface-container-high/30 transition-colors group';
          ...
          const changeClass = changePercent !== null ? (changePositive ? 'text-green-600' : 'text-red-500') : 'text-on-surface-variant dark:text-dark-on-surface-variant';

          row.innerHTML = `
            <div class="flex items-center gap-sm">
              <div class="w-10 h-10 bg-primary-container/20 dark:bg-dark-primary-container/20 rounded-lg flex items-center justify-center font-bold text-primary dark:text-dark-primary">${item.symbol.slice(0, 2)}</div>
              <div>
                <p class="text-label-md font-bold dark:text-dark-on-surface">${item.symbol}</p>
                <p class="text-[10px] text-on-surface-variant dark:text-dark-on-surface-variant">${priceStr}</p>
              </div>
            </div>
    ...
    function renderSectorExposure(exposure, baseCurrency = 'INR') {
      ...
      if (!exposure || exposure.length === 0) {
        container.className = "h-64 w-full flex items-center justify-center border border-dashed border-outline-variant/30 rounded-xl bg-surface-container-lowest/50 dark:bg-dark-surface-container/50";
        container.innerHTML = `<p id="sector-empty-state" class="text-label-md text-on-surface-variant dark:text-dark-on-surface-variant text-center">Add holdings below to see your sector exposure breakdown.</p>`;
        return;
      }
      
      container.className = "w-full space-y-4 py-4 px-2 flex flex-col justify-start max-h-64 custom-scrollbar overflow-y-auto";

      // Distinct, dark-mode-aware color per sector so bars are easy to tell apart
      // at a glance instead of all rendering as the same flat lavender.
      const SECTOR_COLORS = [
        'bg-[#6366f1] dark:bg-[#c0c1ff]',   // indigo / lavender
        'bg-[#0891b2] dark:bg-[#4cd7f6]',   // cyan / teal
        'bg-[#d97706] dark:bg-[#ffb95f]',   // amber / orange
        'bg-[#db2777] dark:bg-[#f472b6]',   // rose / pink
        'bg-[#16a34a] dark:bg-[#4ade80]',   // green / mint
        'bg-[#7c3aed] dark:bg-[#a78bfa]'    // violet / light violet
      ];
      ...
      sorted.forEach((item, index) => {
        ...
        const barColorClass = SECTOR_COLORS[index % SECTOR_COLORS.length];
        sectorRow.innerHTML = `
          <div class="flex justify-between items-center text-label-sm font-medium">
            <span class="text-on-surface dark:text-dark-on-surface truncate pr-2">${item.sector}</span>
            <span class="text-on-surface-variant dark:text-dark-on-surface-variant shrink-0">${marketValueStr} (${item.percentage.toFixed(1)}%)</span>
          </div>
          <div class="w-full bg-slate-100 dark:bg-slate-800 h-2 rounded-full overflow-hidden">
            <div class="${barColorClass} h-full rounded-full transition-all duration-500 ease-out" style="width: ${item.percentage}%"></div>
          </div>
        `;
    ...
    function renderNews(listElement, newsItems, emptyText, emptyTextEl) {
      ...
      if (!newsItems || newsItems.length === 0) {
        if (emptyTextEl) {
          ...
          listElement.innerHTML = `<p class="text-label-sm text-on-surface-variant dark:text-dark-on-surface-variant">${emptyText}</p>`;
        }
        return;
      }
      ...
      newsItems.forEach(item => {
        ...
        card.className = 'glass-card p-sm flex gap-md items-center group cursor-pointer hover:shadow-md transition-all bg-white dark:bg-dark-surface-container border border-outline-variant/30 rounded-xl hover-lift';
        ...
        card.innerHTML = `
          ...
          <div class="flex-grow min-w-0">
            <div class="flex gap-2 mb-1 items-center">
               ...
               <span class="text-[10px] text-on-surface-variant dark:text-dark-on-surface-variant">${item.createdAt ? new Date(item.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : 'Today'}</span>
            </div>
      ...
      async function showSuggestions() {
        ...
        filtered.forEach((item, idx) => {
          ...
          itemEl.innerHTML = `
            ...
            <span class="text-[11px] text-on-surface-variant dark:text-dark-on-surface-variant truncate ml-4 text-right">${item.name} (${item.exchange || ''})</span>
          `;
    ...
    function renderMarketHeadlines(news) {
      ...
      if (news.length === 0) {
        ticker.innerHTML = '<span class="text-on-surface-variant dark:text-dark-on-surface-variant font-medium select-none">No recent market headlines available.</span>';
        return;
      }
      ...
      itemsToDisplay.forEach((item, index) => {
        ...
        link.className = 'hover:underline text-on-surface dark:text-dark-on-surface font-semibold flex items-center gap-2 shrink-0';
        
        if (index > 0) {
          const dot = document.createElement('span');
          dot.textContent = '•';
          dot.className = 'text-on-surface-variant dark:text-dark-on-surface-variant select-none mx-1';
      ...
      gapDot.textContent = '•';
      gapDot.className = 'text-on-surface-variant dark:text-dark-on-surface-variant select-none mx-2 shrink-0';
    ...
    if (tabHoldings && tabWatchlist) {
      tabHoldings.addEventListener('click', () => {
        tabHoldings.className = 'flex-1 py-4 text-center font-bold text-label-md border-b-2 border-primary dark:border-dark-primary text-primary dark:text-dark-primary transition-all';
        tabWatchlist.className = 'flex-1 py-4 text-center font-bold text-label-md border-b-2 border-transparent text-on-surface-variant dark:text-dark-on-surface-variant hover:text-primary dark:hover:text-inverse-primary transition-all';
        paneHoldings.classList.remove('hidden');
        paneWatchlist.classList.add('hidden');
      });

      tabWatchlist.addEventListener('click', () => {
        tabWatchlist.className = 'flex-1 py-4 text-center font-bold text-label-md border-b-2 border-primary dark:border-dark-primary text-primary dark:text-dark-primary transition-all';
        tabHoldings.className = 'flex-1 py-4 text-center font-bold text-label-md border-b-2 border-transparent text-on-surface-variant dark:text-dark-on-surface-variant hover:text-primary dark:hover:text-inverse-primary transition-all';
    ...
    function renderDetailsContent(data) {
      ...
      sortedSectors.forEach((item, idx) => {
        ...
        sectorDiv.innerHTML = `
          <div class="flex justify-between items-center text-label-sm font-medium px-1">
            <div class="flex items-center gap-1">
              <span class="material-symbols-outlined text-[16px] text-on-surface-variant dark:text-dark-on-surface-variant group-hover:text-primary transition-transform duration-200" id="chevron-sec-${idx}">chevron_right</span>
              <span class="text-on-surface dark:text-dark-on-surface">${item.sector}</span>
            </div>
            <span class="text-on-surface-variant dark:text-dark-on-surface-variant">${formatMoneyWithIntl(item.value, baseCurr)} (${item.percentage.toFixed(1)}%)</span>
          </div>
        ...
        constituentsDiv.className = 'hidden pl-6 pr-2 py-2 space-y-1.5 bg-surface-container-low/30 dark:bg-dark-surface-container-high/10 rounded-lg mt-2 text-xs border-t border-outline-variant/10';
        ...
        sectorHoldings.forEach(sh => {
          ...
          itemRow.className = 'flex justify-between items-center text-[11px] text-on-surface-variant dark:text-dark-on-surface-variant';
          itemRow.innerHTML = `
            <span>
              <span class="font-bold text-on-surface dark:text-dark-on-surface">${sh.symbol}</span> 
              <span class="opacity-85">(${sh.companyName || sh.symbol})</span>
            </span>
            <div class="text-right">
              <span class="font-semibold text-on-surface dark:text-dark-on-surface">${formatMoneyWithIntl(sh.marketValue, sh.currency)}</span>
      ...
      data.holdings.forEach(h => {
        ...
        tr.innerHTML = `
          <td class="p-sm font-bold text-on-surface dark:text-dark-on-surface">${h.symbol}</td>
          <td class="p-sm text-on-surface-variant dark:text-dark-on-surface-variant truncate max-w-[150px]" title="${h.companyName || h.symbol}">${h.companyName || h.symbol}</td>
          <td class="p-sm text-xs text-on-surface-variant dark:text-dark-on-surface-variant">${h.sector}</td>
          <td class="p-sm text-right text-on-surface dark:text-dark-on-surface">${h.quantity}</td>
          <td class="p-sm text-right text-on-surface-variant dark:text-dark-on-surface-variant">${formatMoneyWithIntl(h.avgCost, h.currency)}</td>
          <td class="p-sm text-right text-on-surface-variant dark:text-dark-on-surface-variant">${currentPriceStr}</td>
          <td class="p-sm text-right font-semibold text-on-surface dark:text-dark-on-surface">${formatMoneyWithIntl(h.marketValue, h.currency)}</td>
      ...
      data.holdings.forEach(h => {
        const shGainPositive = h.gain >= 0;
        const card = document.createElement('div');
        card.className = 'p-sm border border-outline-variant/20 rounded-xl bg-white dark:bg-dark-surface-container-low space-y-2';
        
        const currentPriceStr = h.currentPrice !== null ? formatMoneyWithIntl(h.currentPrice, h.currency) : 'N/A';
        const gainPercentStr = h.gainPercent !== null ? `${shGainPositive ? '+' : ''}${h.gainPercent.toFixed(2)}%` : 'N/A';
        const gainClass = h.gainPercent !== null ? (shGainPositive ? 'text-green-600' : 'text-red-500') : 'text-on-surface-variant dark:text-dark-on-surface-variant';

        card.innerHTML = `
          <div class="flex justify-between items-center border-b border-outline-variant/10 pb-1.5">
            <div>
              <span class="font-bold text-label-md text-on-surface dark:text-dark-on-surface">${h.symbol}</span>
              <span class="text-[10px] text-on-surface-variant dark:text-dark-on-surface-variant ml-2">${h.companyName || h.symbol}</span>
            </div>
            <span class="text-xs bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded text-on-surface-variant dark:text-dark-on-surface-variant">${h.sector}</span>
          </div>
          <div class="grid grid-cols-2 gap-y-1 text-xs">
            <div class="text-on-surface-variant dark:text-dark-on-surface-variant">Qty: <span class="text-on-surface dark:text-dark-on-surface font-semibold">${h.quantity}</span></div>
            <div class="text-on-surface-variant dark:text-dark-on-surface-variant text-right">Avg Cost: <span class="text-on-surface dark:text-dark-on-surface font-semibold">${formatMoneyWithIntl(h.avgCost, h.currency)}</span></div>
            <div class="text-on-surface-variant dark:text-dark-on-surface-variant">Current: <span class="text-on-surface dark:text-dark-on-surface font-semibold">${currentPriceStr}</span></div>
            <div class="text-on-surface-variant dark:text-dark-on-surface-variant text-right">Value: <span class="text-on-surface dark:text-dark-on-surface font-bold">${formatMoneyWithIntl(h.marketValue, h.currency)}</span></div>
          </div>
          <div class="flex justify-between items-center text-xs pt-1 border-t border-outline-variant/10">
            <span class="text-on-surface-variant dark:text-dark-on-surface-variant">Total Gain/Loss:</span>
